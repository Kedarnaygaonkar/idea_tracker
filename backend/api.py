...
- Use plain text formatting, no markdown"""
    ...
    try:
        ...
        # Gemini expects a simple prompt string; we'll concatenate history
        conversation_parts = [system_prompt + "\n\n"]

        # Add conversation history (last 10 turns max to stay within token limits)
        recent_history = history[-10:] if len(history) > 10 else history
        for msg in recent_history:
            role_label = "Researcher" if msg.get("role") == "user" else "Assistant"
            conversation_parts.append(f"{role_label}: {msg.get('content', '')}\n\n")

        # Add current message
        conversation_parts.append(f"Researcher: {user_message}\n\nAssistant:")
